...
import com.delivery_api.Projeto.Delivery.API.dto.StatusUpdateDTO;
...
public class PedidoControler {
     ...
     */
    @PatchMapping("/{pedidoId}/status") // ⬅️ Nova Rota: /pedidos/1/status
    public ResponseEntity<?> atualizarStatus(@PathVariable Long pedidoId,
                                             @RequestBody StatusUpdateDTO statusUpdateDTO) { // ⬅️ Recebe o status no corpo
        try {
            // Chama o Service com o status extraído do DTO
            Pedido pedido = pedidoService.atualizarStatus(pedidoId, statusUpdateDTO.getStatus());
            return ResponseEntity.ok(pedido);
